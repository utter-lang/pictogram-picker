...
class TextSymbolDialog(ctk.CTkToplevel):
    ...
    def __init__(self, master, text_to_render, *args, **kwargs):
        ...
        if not self.font_names:
            messagebox.showwarning("Font Not Found", "Could not find any system fonts.")
        ...
        preferred_fonts = ["Arial", "DejaVu Sans"]
        default_font = "Arial"
        ...
        ctk.CTkLabel(controls_frame, text="Symbol Size:").grid(
            row=1, column=0, padx=5, pady=5
        )
        self.font_size_var = ctk.IntVar(value=350)
        ...
        try:
            ...
            for font_path in findSystemFonts():
                try:
                    ...
                    if font.style_name.lower() in ["regular", "normal", "book"]:
                        if font.family_name not in font_map:
                            font_map[font.family_name] = font_path
                except Exception:
                    continue
            return dict(sorted(font_map.items()))
        except ImportError:
            print("Warning: matplotlib not found. Using basic font list.")
            return {
                "Arial": "arial.ttf",
    ...
    def update_preview(self, *args):
        # 1. Define a higher resolution for rendering and a separate size for display
        RENDER_SIZE = 2048  # Render a large image for quality
        DISPLAY_SIZE = self.img_size  # Your original 512px for the GUI preview

        # Get the font size from the slider, but we'll scale it for the high-res render
        base_font_size = self.font_size_var.get()

        # 2. Scale the font size to match the larger render canvas
        # This maintains the text-to-image size ratio you see in the preview
        render_font_size = int(base_font_size * (RENDER_SIZE / DISPLAY_SIZE))
        ...
        try:
            ...
            print(
                f"Warning: Could not load font '{selected_font_name}'. Falling back to default."
            )
            font = ImageFont.load_default(size=render_font_size)

        # 3. Render the text onto the HIGH-RESOLUTION canvas
        # We use the two-pass method to correctly center the content
        try:
            # Pass 1: Render on a temporary surface to get the true pixel size
            temp_image = Image.new(
                "RGBA", (RENDER_SIZE * 2, RENDER_SIZE * 2), (0, 0, 0, 0)
            )
            with Pilmoji(temp_image) as pilmoji:
                pilmoji.text((0, 0), self.text_to_render, (0, 0, 0), font=font)

            bbox = temp_image.getbbox()
            if not bbox:
                bbox = (0, 0, 0, 0)

            cropped_image = temp_image.crop(bbox)
            text_width, text_height = cropped_image.size

            # Pass 2: Create the final high-res image and paste the content in the center
            high_res_image = Image.new(
                "RGBA", (RENDER_SIZE, RENDER_SIZE), (255, 255, 255, 255)
            )
            x = (RENDER_SIZE - text_width) // 2
            y = (RENDER_SIZE - text_height) // 2
            high_res_image.paste(cropped_image, (x, y), cropped_image)

        except Exception as e:
            print(f"Error during pilmoji rendering: {e}")
            # Create a fallback error image if rendering fails
            high_res_image = Image.new(
                "RGBA", (RENDER_SIZE, RENDER_SIZE), (255, 255, 255, 255)
            )
            draw = ImageDraw.Draw(high_res_image)
            error_font = ImageFont.load_default(size=100)
            draw.text((50, 50), "Error rendering symbol.", fill="red", font=error_font)

        # 4. Store the full, high-resolution image for the final save operation
        self.final_image = high_res_image

        # 5. Create a downscaled, high-quality copy for the GUI preview
        display_image = high_res_image.copy()
        display_image.thumbnail((DISPLAY_SIZE, DISPLAY_SIZE), Image.Resampling.LANCZOS)

        ctk_image = ctk.CTkImage(
            light_image=display_image, size=(DISPLAY_SIZE, DISPLAY_SIZE)
        )
...
class SymbolPickerPage:
    def __init__(self, master, controller):
        ...
        try:
            ...
            if os.path.isdir(bliss_path):
                ...
                print(f"Warning: Bliss symbol directory '{bliss_path}' not found.")
    ...
    def _find_system_font(self):
        ...
        if system == "Windows":
            ...
            for path in paths:
                if os.path.exists(path):
                    return path
        print("Warning: Could not find a default system font for separator.")
        return None

    def get_code_from_pasted_emoji(self):
        pasted_text = self.emoji_symbol_entry.get()
        if not pasted_text:
            messagebox.showinfo("Info", "Please paste an emoji into the box first.")
            return
        try:
            emoji_char = pasted_text[0]
            char_code = ord(emoji_char)
            hex_code = hex(char_code)[2:].upper()
            self.emoji_symbol_entry.delete(0, "end")
            self.emoji_symbol_entry.insert(0, hex_code)
        except (TypeError, ValueError):
            messagebox.showerror(
                "Error",
                "Could not read the pasted emoji. It might not be a valid single character.",
            )
        except Exception as e:
            messagebox.showerror("Error", f"An unexpected error occurred: {e}")

    def create_emoji_symbol(self):
        user_input = self.emoji_symbol_entry.get().strip()
        if not user_input:
            return

        try:
            parsed_char = chr(int(user_input, 16))
        except (ValueError, TypeError):
            messagebox.showerror(
                "Invalid Input",
                "Please enter a valid Unicode Hex Code (e.g., 1F600).\nUse the 'Get Code' button if you pasted an emoji.",
            )
            return

        dialog = TextSymbolDialog(self.root, text_to_render=parsed_char)
        final_image = dialog.get_result()
        if final_image:
            symbol_info = {"name": parsed_char, "original_filename": "custom_emoji.png"}
            if self.multi_select_mode.get():
                self.add_to_selection(
                    symbol_info, "Custom Emoji", final_image, "image_obj"
                )
            else:
    ...
    def combine_and_save_symbols(self):
        ...
        try:
            font = (
                ImageFont.truetype(self.separator_font_path, 200)
    ...
    def setup_gui(self):
        ...
        self.custom_search_entry.grid(row=0, column=1, columnspan=2, sticky="ew")
        ...
        self.text_symbol_entry = ctk.CTkEntry(
            search_controls_frame, width=int(ENTRY_WIDTH * 0.7 * UI_SCALE)
        )
        self.text_symbol_entry.grid(row=1, column=1, sticky="ew", pady=(5, 0))
        ...
        self.create_text_symbol_button = ctk.CTkButton(
            search_controls_frame,
            text="Create",
            command=self.create_text_symbol,
            width=int(ENTRY_WIDTH * 0.3),
        )
        self.create_text_symbol_button.grid(
            row=1, column=2, sticky="w", pady=(5, 0), padx=5
        )
        ctk.CTkLabel(
            search_controls_frame, text="Emoji Symbol:", font=self.normal_font
        ).grid(
            row=2,
            column=0,
            sticky="w",
        ...
        self.emoji_symbol_entry = ctk.CTkEntry(
            search_controls_frame,
            placeholder_text="Paste emoji -> Get Code -> Create",
            width=int(ENTRY_WIDTH * 0.7 * UI_SCALE),
        )
        self.emoji_symbol_entry.grid(row=2, column=1, sticky="ew", pady=(5, 0))
        self.emoji_symbol_entry.bind("<FocusIn>", self.disable_root_key_bindings)
        self.emoji_symbol_entry.bind("<FocusOut>", self.enable_root_key_bindings)
        emoji_buttons_frame = ctk.CTkFrame(
            search_controls_frame, fg_color="transparent"
        )
        emoji_buttons_frame.grid(row=2, column=2, sticky="w", pady=(5, 0), padx=5)
        self.get_code_button = ctk.CTkButton(
            emoji_buttons_frame,
            text="Get Code",
            command=self.get_code_from_pasted_emoji,
            width=int(ENTRY_WIDTH * 0.15),
        )
        self.get_code_button.pack(side="left", padx=(0, 5))
        self.create_emoji_button = ctk.CTkButton(
            emoji_buttons_frame,
            text="Create",
            command=self.create_emoji_symbol,
            width=int(ENTRY_WIDTH * 0.15),
        )
        self.create_emoji_button.pack(side="left")
    ...
    def _commit_symbol(self, filename, symbol_name, source, symbol_data_json):
        ...
        ] = [filename, symbol_name, source, symbol_data_json, ""]
        self.auto_save()
    ...
    def show_existing_symbol(self):
        ...
        try:
            ...
            ctk_image = ctk.CTkImage(
                light_image=final_image_for_display, size=(final_width, final_height)
            )
